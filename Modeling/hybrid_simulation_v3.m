...
%   the first phase (stance) ended
    ...
    for i = 1:num_step-1
        ...
        [dz, u] = dynamics_continuous(t,zout(:,i),ctrl,p,iphase,max_y);
...
%% Continuous dynamics
function [dz, u] = dynamics_continuous(t,z,ctrl,p,iphase,max_y)
    ...
    % Vertical force on BC member to simulate assist
    % TODO
...
%     QTau_Fh = F_Ax * Jhx.';
    ...
    QTau_Fhy = 0;
%     % vertical spring damper on hand (limit constraint)

%     min_vsep = 0.3175;
%     Khv = 10;
%     Dhv = .1;
%     if (rA(2) - rfoot < min_vsep) 
%         Jhy = Jh(2,:);
%         yerr = max_y + min_vsep - rA(2);
%         dyerr = 0 - drA(2);
%         F_Ay = max(0, Khv * yerr + Dhv * dyerr);
%         QTau_Fhy =  F_Ay * Jhy.';
%     end
...
%% Joint Limit Constraint - Impulse (not in use)
% Constraint s.t. q...
...
function QTauc = joint_limit_torque(z,p)
    ...
    q2_ulim = 20/360*2*pi;
    q3_llim = 30/360*2*pi;   % Theta 2 
    q3_ulim = 130/360*2*pi;

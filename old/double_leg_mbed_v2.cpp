...
//    pc.printf("%f",input_params[0]);
    ...
    while(1) {
        ...
        if (server.getParams(input_params,NUM_INPUTS)) {
            ...
            angle1R_init                = input_params[3];    // Initial angle for q1 (rad)
            angle2R_init                = input_params[4];    // Initial angle for q2 (rad)
            angle1L_init                = angle1R_init;
            angle2L_init                = -1.0*angle2R_init;  // TODO -- MAKE THESE INPUT PARAMS!
            ...
            for(int i = 0; i<2*(BEZIER_ORDER_FOOT+1);i++) {  // Right foot
              foot_ptsR[i] = input_params[13+i];    
            }
            for(int i = 0; i<2*(BEZIER_ORDER_FOOT+1);i++) {  // TODO -- mod with new inputs instead of copying? Or phase offset
              float bpt = input_params[13+i];
              if (i <= 7) {  // Flip x-value signs for the left side
                bpt *= -1.0;
              }
              foot_ptsL[i] = bpt;    
            }
